...
*/
	...
	while(!done) {
		...
		if(!chckmiss) {
			...
			val[to] = (((obj*)self_md->multidval.val)[from] == missing->intval) ? missing->intval:((obj*)self_md->multidval.val)[from];
		}
...
*/
	...
	output_md = (NclData)_NclMultiDVallistDataCreate(NULL,
		NULL,
                Ncl_MultiDVallistData,
                0,(void*)val,	
		tmp,
...
static NhlErrorTypes MultiDVal_list_md_WriteSection
#if	NhlNeedProto
...
static NhlErrorTypes MultiDVal_list_s_WriteSection
#if	NhlNeedProto
(NclData target, NclSelectionRecord * sel, struct _NclDataRec* value)
#else
(target,sel,value)
...
*/
	...
	while(!done) {
		...
		if((target_md->multidval.missing_value.has_missing)&&(target_md->multidval.missing_value.value.objval != ((obj*)target_md->multidval.val)[to])) {
			_NclDelParent((NclObj)_NclGetObj(((obj*)target_md->multidval.val)[to]),(NclObj)target_md);
		} else {
			_NclDelParent((NclObj)_NclGetObj(((obj*)target_md->multidval.val)[to]),(NclObj)target_md);
		}
...
static void MultiDVal_list_ResetMissing
#if	NhlNeedProto
...
static NhlErrorTypes MultiDVal_list_ReadWriteSection
#if	NhlNeedProto
...
static void MultiDVal_list_Destroy
#if	NhlNeedProto
(NclObj self)
#else
(self)
...
#endif
{
	...
	for(i = 0; i< self_md->multidval.totalelements; i++) {
		tmp_list = (NclList)_NclGetObj(obj_ids[i]);
		(void)_NclDelParent((NclObj)tmp_list,self);
	}
...
static NclData NclMultiDVallistDup
#if	NhlNeedProto
...
#endif
{
	...
	if(self_md->multidval.val != NULL) {
		if((self_md->multidval.missing_value.has_missing)&&(self_md->multidval.missing_value.value.objval == *(obj*)self_md->multidval.val)) {
			...
			if(new_missing == NULL) {
				*toval = self_md->multidval.missing_value.value.objval;
				themissing = self_md->multidval.missing_value.value;
			} else {
				*toval = new_missing->objval;
				themissing = *new_missing;
			}
		} else {
			if((self_md->multidval.missing_value.has_missing)&&(new_missing == NULL)) {
				themissing = self_md->multidval.missing_value.value;
			} else if(new_missing != NULL) {
				themissing = *new_missing;
			}
			fr_list = (NclList)_NclGetObj(*(obj*)self_md->multidval.val);
			if(fr_list == NULL) {
				return(NULL);
			}
			to_list = _NclListSelect(fr_list,NULL);
			if(to_list != NULL) {
				toval = (obj*)NclMalloc(sizeof(obj));
				*toval = to_list->obj.id;
			} else {
				return(NULL);
			}
		}
        ...
        return((NclData)_NclMultiDVallistDataCreate(
                NULL,
                NULL,
                Ncl_MultiDVallistData,
                0,
...
static NhlErrorTypes InitializelistDataClass(
#if NhlNeedProto
void
#endif
);

NclMultiDVallistDataClassRec nclMultiDVallistDataClassRec = {
	{
/* char *class_name; 		*/	"MultiDVallistData",
/* unsigned int obj_size;	*/	sizeof(NclMultiDVallistDataRec),
/* NclObjClass 			*/	(NclObjClass)&nclMultiDValDataClassRec,
/* int inited			*/	0,
/* NclGenericFunction destroy; 	*/	MultiDVal_list_Destroy,
...
/* NclObtainCall obtain_calldata*/   NULL
	...
	{
/* NclGenericFunction dup; 	*/	NclMultiDVallistDup,
/* NclResetMissingValueFuction dup;	*/	MultiDVal_list_ResetMissing,
/* NclReadSubSecFunction r_subsection */ MultiDVal_list_ReadSection,
/* NclReadSubSecFunction w_subsection */{
					MultiDVal_list_md_WriteSection,
					MultiDVal_list_s_WriteSection
					},
/* NclReadThenWriteSubFunc w_subsection */ MultiDVal_list_ReadWriteSection,
...
NclObjClass nclMultiDVallistDataClass = (NclObjClass)&nclMultiDVallistDataClassRec;
...
#endif
{
	_NclRegisterClassPointer(
		Ncl_MultiDVallistData,
		(NclObjClass)&nclMultiDVallistDataClassRec
	);
	return(NhlNOERROR);
}
...
#endif
{
	NclMultiDVallistData thevalobj;
	NclObjClass class_ptr= nclMultiDVallistDataClass;
	int i;
	NhlErrorTypes ret1= NhlNOERROR;
	int *obj_ids;
	NclList tmp_list;

	ret1 = _NclInitClass(nclMultiDVallistDataClass);
	if(ret1 < NhlWARNING) {
		return(NULL);
	}
	if(inst == NULL ) {
		thevalobj = (NclMultiDVallistData)NclMalloc(
				(unsigned)nclMultiDVallistDataClassRec.obj_class.obj_size);
	} else {
		thevalobj = (NclMultiDVallistData)inst;
	}
	if(theclass != NULL) {
		class_ptr = theclass;
	}
...
*/
	...
	_NclCreateMultiDVal((NclObj)thevalobj,class_ptr,obj_type,(obj_type_mask | Ncl_MultiDVallistData),val,missing_value,n_dims,dim_sizes,status,sel_rec,(NclTypeClass)nclTypelistClass);


	thevalobj->multidval.data_type = NCL_list;

	obj_ids = (obj*)thevalobj->multidval.val;
	for(i = 0; i<thevalobj->multidval.totalelements; i++) {
		tmp_list = (NclList)_NclGetObj(obj_ids[i]);
		if((tmp_list != NULL)&&(tmp_list->obj.obj_type_mask & Ncl_List)){
			(void)_NclAddParent((NclObj)tmp_list,(NclObj)thevalobj);
		}
	}
