/*
 *      $Id: NclAdvancedOGR.c 14556 2013-09-19 20:59:24Z huangwei $
 */
/************************************************************************
*    								*
*    		     Copyright (C)  2009			*
...
static void *AdvancedOGROpenFile(void *therec, NclQuark path, int wr_status)
{
    ...
    ng_size_t geometryCounter = 1024;
    ng_size_t *geometryRecord = (ng_size_t *)NclCalloc(geometryCounter, sizeof(ng_size_t));
  ...
  /*Read through the geometry to get the number of features, segments, and points...*/
	...
	{
            geometryCounter *= 2;
            geometryRecord = (ng_size_t *)NclRealloc(geometryCounter, geometryCounter*sizeof(ng_size_t));
            if(NULL == geometryRecord)
            {
	        NhlPError(NhlFATAL, NhlEUNKNOWN,
