...
/* grid type */
		...
		*tmp_string = step->var_info.long_name_q;		
		...
		if (Grib2ReadCodeTable(grib_rec->table_source, 4, "4.3.table", grib_rec->traits.gen_process_type,-1,ct) < NhlWARNING) {
			return;
		}
		tmp_string = (NclQuark*)NclMalloc(sizeof(NclQuark));
		if (ct->descrip) {
			*tmp_string = NrmStringToQuark(ct->descrip);
			...
			sprintf(buf,"%d",grib_rec->traits.gen_process_type);
			*tmp_string = NrmStringToQuark(buf);
		}
		att_list_ptr->att_inq->thevalue = (NclMultiDValData)
			_NclCreateVal(NULL, NULL,
